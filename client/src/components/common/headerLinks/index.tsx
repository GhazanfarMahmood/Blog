    ...
    return <ul className="flex items-center justify-center gap-2.5 z-[2]">
                {links.map((item) => {
                    return <li key={item.id} className={`${item.category ? "relative" : ""} group`}>
                        <Link href={"/"} className={`p-[9px_14px] flex justify-center items-center gap-1.5 capitalize font-medium text-primary rounded-lg transition-all duration-[.25s] ease-in group-hover:bg--link-bg`} >
                            {item.name}
                            {item.category && 
                            ...
                            <ul className="w-[234px] py-3.5 pl-3 pr-4 absolute top-11 bg-light rounded-lg shadow-links transition-all duration-[0.25s] ease-in invisible opacity-0 group-hover:visible group-hover:opacity-100 after:w-[234px] after:h-3.5 after:absolute after:-top-2.5 after:content-[''] after:bg-transparent">
                                {item.category?.map((items) => {
                                    return  <li key={items.id}>
                                        <Link href={"/"} className="block w-[97%] font-medium capitalize leading-6 py-2 px-4 rounded-lg hover:bg--link-bg transition-all duration-[0.25s] ease-in hover:ml-2">{items.name}</Link>
                                    </li>      
                                })}
                            </ul>
                        }
