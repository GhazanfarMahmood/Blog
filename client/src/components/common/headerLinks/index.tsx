...
export function HeaderLink({activeHeader, setActiveHeader}){
    ...
    return <div className={`max-w-[430px] flex flex-col justify-start lg:max-w-none w-full h-[100vh] lg:h-fit fixed top-0 -left-full lg:static bg-light lg:bg-transparent z-[4] transition-all duration-[0.4s] ease-in ${activeHeader && "left-0"}`}>
        <div className="h-[88px] flex items-center justify-between pr-6 pl-4 lg:hidden">
           ...
           <button aria-label="close-search" className="w-10 h-10 cursor-pointer flex items-center justify-center text-phara hover:text-primary transition-all duration-[0.25s] ease-in" onClick={() => setActiveHeader(false)}>
        ...
        <ul className="h-[calc(100vh-169px)] lg:h-fit flex items-start lg:items-center justify-start lg:justify-center flex-col lg:flex-row gap-1 lg:gap-2.5 z-[2] px-1.5 overflow-y-auto lg:overflow-y-visible">
            {links.map((item) => {
                return <li key={item.id} className={`w-full lg:w-fit group ${item.category ? "relative" : ""}`}>
                    <Link href={"/"} className={`w-full lg:w-fit p-[6px_14px] font-semibold flex justify-start lg:justify-center items-center gap-1.5 capitalize text-primary rounded-lg transition-all duration-[.25s] ease-in lg:group-hover:bg--link-bg`} onClick={handleWindowWidth} >
                        {item.name}
                        {item.category && <>
                            <FaChevronDown className={`hidden lg:block w-3 h-3 transition-all duration-[.25s] ease-in lg:group-hover:-rotate-180`} />
                            <FaChevronDown className={`w-3 h-3 transition-all duration-[.25s] ease-in block lg:hidden ml-auto ${activeLinks && "-rotate-180"}`} />
                            </>
                        }
                    </Link>
                    {activeLinks &&
                    (item.category &&
                        <ul 
                        className="w-full lg:w-[234px] py-1 lg:py-3.5 pl-3 pr-4 static lg:absolute top-11 bg-light rounded-lg shadow-none lg:shadow-links transition-all duration-[0.25s] ease-in 
                        visible lg:invisible lg:group-hover:visible opacity-100 lg:opacity-0 group-hover:opacity-100 
                        
                        after:w-[234px] after:h-3.5 after:absolute after:-top-2.5 after:content-none lg:after:content-[''] after:bg-transparent"
                        >
                            {item.category?.map((items) => {
                                return  <li key={items.id}>
                                    <Link href={"/"} className="text-phara lg:text-primary block w-[97%] font-semibold capitalize leading-6 py-1 lg:py-2 px-4 rounded-lg hover:bg--link-bg transition-all duration-[0.25s] ease-in hover:ml-2">{items.name}</Link>
                                </li>      
                            })}
                        </ul>
                        )
        ...
        <div className="flex items-center justify-between lg:hidden py-2 px-6 border-t border-t-br">
            ...
            <button className="flex lg:hidden items-center justify-center gap-2 ml-3.5 p-1 rounded-[30px] cursor-pointer *:w-7 *:h-7 *:flex *:items-center *:justify-center bg--link-bg *:transition-opacity *:duration-75 *:ease-in *:hover:opacity-70" aria-label="theme toggle btn">
                <span>
                    <MdWbSunny />   
                </span>
