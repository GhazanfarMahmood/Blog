...
export default function CategoryDetailContent(){
    ...
    return <>
        ...
        <div className="container">
            ...
            >
                {blogs?.map((item) => {
                    return <BlogCard title={item.title} img={item.thumbnail} category={item.category} reading={item.reading} authorName={item.author} date={item.createdAt} description={item.excerpt} slug={item.slug} key={item._id} />
                })}
            </div>
        </div>
    </>
